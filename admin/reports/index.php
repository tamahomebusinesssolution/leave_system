        ...
        <div class="container-fluid" id="print_out">
			<table class="table table-hover table-stripped">
				...
				<tbody>
                    ...
                    where ((date(l.date_start) BETWEEN '$date_start'  and '$date_end') OR (date(l.date_end) BETWEEN '$date_start'  and '$date_end') ) order by unix_timestamp(l.date_start) asc,unix_timestamp(l.date_end) asc";
						$qry = $conn->query($sql);
						while($row = $qry->fetch_assoc()):
                            $lt_qry = $conn->query("SELECT meta_value FROM `employee_meta` where user_id = '{$row['user_id']}' and meta_field = 'employee_id' and meta_field = 'department_id' ");
							$row['employee_id'] = ($lt_qry->num_rows > 0) ? $lt_qry->fetch_array()['meta_value'] : "N/A";
                            $row['department_id'] = ($lt_qry->num_rows > 0) ? $lt_qry->fetch_array()['meta_value'] : "N/A";
					?>
						<tr>
							<td class="text-center"><?php echo $i++; ?></td>
							<td>
                                <small><b>ID: </b><?php echo $row['emp_id'] ?></small><br>
